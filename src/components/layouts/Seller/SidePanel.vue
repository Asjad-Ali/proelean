      ...
      <nav id="app-nav-main" class="app-nav app-nav-main flex-grow-1">
        <ul class="app-menu list-unstyled accordion" id="menu-accordion">
           ...
           <li class="nav-item" v-for="(item,index) in items" :key="index">
            ...
            <router-link  class="nav-link" :class="{active: item.path == $route.path}" :to="item.path" @click="handleCloseSideBar(item.path)" >
              <span class="nav-icon">
                <svg
                  width="1em"
                  height="1em"
                ...
                >
                  <path v-for="(path,index) in item.svgPaths" :key="index"
                    fill-rule="evenodd"
                    :d="path"
                  />
                </svg>
              </span>
              <span class="nav-link-text">{{item.title}}</span> </router-link
            ><!--//nav-link-->
          </li>
        </ul>
        <!--//app-menu-->
